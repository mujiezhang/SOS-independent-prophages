...
df1=read.table('Supplementary Fig1-Ecoli-K12-HI-predict-all.tsv',head=TRUE,sep='\t')
df2=read.table('Supplementary Fig1-25point-all.tsv',head=TRUE,sep='\t')
...
p1 <- ggplot() +
  ...
  scale_fill_manual(values = c('yes'="#accde8",'no'="#ECB477")) # 使用 scale_color_manual
p1
