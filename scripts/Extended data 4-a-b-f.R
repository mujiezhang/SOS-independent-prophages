...
df1=read.table('Extended data 4-a-b-SiPs-SdPs-length-completeness.txt',head=TRUE,sep='\t')
df2=read.table('Extended data4-f-protein-function.txt',head=TRUE,sep='\t')
df2$type <- factor(df2$type,levels =  c('SiPs-L','SdPs-L','SiPs-S','SdPs-S'))
df2$func <- factor(df2$func,levels = c('integrase','terminase','portal-associated','lysis-associated','tail- and baseplate-associated'))

df1$type2 <- factor(df1$type2,levels = c('SiPs','SdPs'))
df1$type <- factor(df1$type,levels =  c('SiPs-L','SdPs-L','SiPs-S','SdPs-S'))
p1<-ggplot(df1, aes(x = contig_length,fill=type2)) +
...
p3=ggplot(df2,aes(x=func,y=percent,fill=type))+
