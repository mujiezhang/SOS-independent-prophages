...
create_violin_plot <- function(file, title = "Coding Density", y_label = "Coding density (kb⁻¹)", show_legend = FALSE) {
  ...
  combined_data$Source <- factor(combined_data$Source, levels = c("SiTVs", "SdTVs"))  
  ...
  p <- ggplot(combined_data, aes(x = Source, y = value, fill = Source)) + 
    ...
    # 添加平均值标记（三角形）和平均值标签
    stat_summary(fun = mean, geom = "point", shape = 17, size = 1, color = "#e05c00", fill = "#e05c00") +  # 三角形表示平均值
    stat_summary(fun = mean, geom = "text", aes(label = round(after_stat(y), 3)), vjust = -0.5, color = "#e05c00") +  # 标注平均值
...
data_sets <- list(
  list(
    file = "1_genomesize_SdTVs-GC_stats_phage_all.txt",  # 合并后的文件路径
    title = "Dataset 1: Genome Size",
    y_label = "Genome size (kb)"
  ),
  list(
    file = "2_GC_SdTVs-GC_stats_phage_all_IR.txt",  # 合并后的文件路径
    title = "Dataset 2: GC of IR",
    y_label = "GC content (%) of IR"
  ),
  list(
    file = "3_SdTVs-coding_density_phage_all.txt",  # 合并后的文件路径
    title = "Dataset 3: Protein Coding Density",
    y_label = "Protein coding density (kb⁻¹)"
  ),
  list(
    file = "5_average_cds_overa_all.txt",  # 合并后的文件路径
    title = "Dataset 4: Percentage of CDS Overlap",
    y_label = "Percentage of CDS overlap"
  ),
  list(
   file = "4_average_protein_size_all.txt",  # 合并后的文件路径
